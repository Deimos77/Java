    ...
    public void run() {
        try {
            ...
            if (sqlConnection.ConnectionToDB()) {
                ...
                if (key.toUpperCase().equals("YES") || key.toUpperCase().equals("ДА")) {
                    ...
                    while (!islogged) {
                        ...
                        if (sqlConnection.userExist(login, password)) {
                            newuser = login;
                            serverSender.send(clientSocket, "Вы успешно авторизованы!Теперь у вас есть доступ к информации о всех фильмах и возможность изменять/добавлять свои фильмы", 0);
                            islogged = true;
                            regist = false;
                        }
                        else {
                ...
                if (key.toUpperCase().equals("NO") || key.toUpperCase().equals("НЕТ") || regist) {
                    ...
                    while (!islogged) {
                        ...
                        if (sqlConnection.addNewUser(login, password)) {
                            serverSender.send(clientSocket, "Вы успешно зарегистрированы,теперь у вас есть доступ к информации о всех фильмах и возможность изменять свои/добавлять свои фильмы", 0);
                            islogged = true;
                            newuser = login;
                            Thread.sleep(20);
                ...
                try {
                    while (true) {
                        System.out.println("Ожидаю команду от : " + clientSocket.getLocalAddress() + clientSocket.getPort());
                        ...
                        System.out.println("Выполняю команду " + commandStringMap.entrySet().iterator().next().getKey().getClass().getName()+" от человечка с адресом: "
                    ...
                    e.printStackTrace();
                    System.out.println("Польхователь с адресом:"+clientSocket.getLocalAddress() + clientSocket.getPort()+" отключился");
                }
            }
        }
